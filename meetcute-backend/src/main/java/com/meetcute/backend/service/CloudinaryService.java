...
public class CloudinaryService {
    ...
    public UploadResult upload(MultipartFile file, String folder) {
        try {
            ...
            String signature = sha1Hex("folder=" + folderParam + "&timestamp=" + timestamp + apiSecret);
            String boundary = "---" + UUID.randomUUID().toString().replace("-", "");

            HttpURLConnection conn = (HttpURLConnection)
                    new URL("https://api.cloudinary.com/v1_1/" + cloudName + "/image/upload").openConnection();
            ...
            try (OutputStream out = conn.getOutputStream();
                ...
                writer.append("Content-Disposition: form-data; name=\"file\"; filename=\"upload.jpg\"\r\n");
            ...
            if (code != 200) throw new RuntimeException("Cloudinary greška: " + response);

            return new UploadResult(extractJson(response, "secure_url"), extractJson(response, "public_id"));
