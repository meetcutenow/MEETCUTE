...
public interface MatchRepository extends JpaRepository<Match, Long> {
    ...
    @Query("SELECT m FROM Match m WHERE m.matchedAt < :before AND m.status = :status")
    List<Match> findByMatchedAtBeforeAndStatus(@Param("before") LocalDateTime before, @Param("status") String status);
}