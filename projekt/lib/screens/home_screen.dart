...
import 'proximity_match_screen.dart';
...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  void _toggleLocation() {
    ...
    final newState = !_locationEnabled;
  ...
  Future<void> _syncActiveStatus(bool active) async {
    try {
      final api = ApiService();
      if (!api.isLoggedIn) return;
      if (active) {
        await api.setActiveStatus(true,
            lat: _userLocation.latitude, lng: _userLocation.longitude);
      } else {
        await api.setActiveStatus(false);
      }
    } catch (_) {}
  }

  Future<void> _startProximityScan() async {
    if (_scanInProgress) return;
    _scanInProgress = true;
    try {
      final api = ApiService();
      if (!api.isLoggedIn) { _scanInProgress = false; return; }
      final candidates = await api.scanNearby(
    ...
    return Row(children: [
      ...
      AnimatedDefaultTextStyle(
        ...
        style: TextStyle(color: primary, fontSize: 13, fontWeight: FontWeight.w700,
            letterSpacing: 0.3, fontFamily: AccessibilityState.fontFamily),
        child: Text(text),
      ),
    ]);
  }
    ...
    return AnimatedContainer(
      ...
      child: Column(children: [
        ...
        _buildMenuItem(index: 1, icon: Icons.coffee_rounded,
            ...
            isLocked: true, onTap: () => _showPremiumDialog(isFilter: false), showDivider: false),
      ]),
    );
  }
