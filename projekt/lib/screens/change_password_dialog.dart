...
class ChangePasswordDialog {
  static void show(BuildContext context, {required bool isCompany}) {
    ...
    final primary  = isDark ? kDarkPrimary : const Color(0xFF700D25);
    final accent   = isDark ? kDarkCardEl  : const Color(0xFFF2E8E9);
    final card     = isDark ? kDarkCard    : Colors.white;
    final onPrimary = isDark ? kDarkBg     : Colors.white;
...
class _ChangePasswordDialogWidget extends StatefulWidget {
  ...
  @override
  State<_ChangePasswordDialogWidget> createState() =>
      _ChangePasswordDialogWidgetState();
}

class _ChangePasswordDialogWidgetState
    extends State<_ChangePasswordDialogWidget> {
  ...
  Future<void> _save() async {
    ...
    if (oldPass.isEmpty) {
      setState(() => _error = 'Unesite staru lozinku.');
      return;
    }
    if (!_hasMin8 || !_hasUpper || !_hasNum) {
      setState(() => _error = 'Nova lozinka ne zadovoljava uvjete.');
      return;
    }
    if (!_matches) {
      setState(() => _error = 'Lozinke se ne podudaraju.');
      return;
    }
    ...
    try {
      final token = widget.isCompany
          ? CompanyAuthState.instance.accessToken!
          : AuthState.instance.accessToken!;

      final endpoint = widget.isCompany
          ? '$_baseUrl/company/auth/password'
          : '$_baseUrl/users/me/password';
      ...
      if (resp.statusCode == 200 && decoded['success'] == true) {
        ...
        setState(() => _error =
            decoded['message'] as String? ?? 'Greška pri promjeni lozinke.');
      }
    } catch (_) {
          ...
          child: SingleChildScrollView(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              Row(children: [
                ...
                Expanded(child: Text('Promjena lozinke',
                    style: TextStyle(
                        color: p,
                        fontSize: 17,
    ...
    void Function(String)? onChanged,
      ...
      Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(14),
          border: Border.all(
              color: primary.withOpacity(0.18), width: 1.2),
          boxShadow: [BoxShadow(
        ...
        child: Row(children: [
          ...
          Icon(Icons.lock_outline_rounded,
              color: primary.withOpacity(0.45), size: 18),
  ...
  Widget _rule(String text, bool ok, Color p) => Row(children: [
    AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
            color: ok ? p : p.withOpacity(0.25), width: 1.4),
      ),
      child: ok
