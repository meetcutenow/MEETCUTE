...
enum NotifType { joined, cancelled, reminder, newEvent, general, chatConsent, chatUnlocked, chatDeclined }
...
class NotificationState {
  ...
  NotificationState._() { _loadRespondedIds(); }
  ...
  void replaceAll(List<AppNotification> notifications) {
    _notifications
      ..clear()
      ..addAll(notifications.map((n) {
        if (_respondedIds.contains(n.id)) n.hasResponded = true;
        return n;
      }));
    _notify();
  }
  ...
  void markResponded(String localId) {
    _respondedIds.add(localId);
    _saveRespondedIds();
    try {
      final notif = _notifications.firstWhere((n) => n.id == localId);
      notif.hasResponded = true;
      _notify();
    } catch (_) {}
  }

  Future<void> _loadRespondedIds() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final ids = prefs.getStringList('responded_notif_ids') ?? [];
      _respondedIds.addAll(ids);
    } catch (_) {}
  }

  Future<void> _saveRespondedIds() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setStringList('responded_notif_ids', _respondedIds.toList());
    } catch (_) {}
  }
    ...
    try {
      ...
      final notifications = list.map((n) {
        ...
        final type = switch (n['type'] as String? ?? '') {
          ...
          'chat_consent_request' => NotifType.chatConsent,
          'chat_unlocked' => NotifType.chatUnlocked,
          'chat_declined' => NotifType.chatDeclined,
          _ => NotifType.general,
        };
                      ...
                      Row(children: [
                        AnimatedDefaultTextStyle(
                          ...
                          style: TextStyle(
                              ...
                              letterSpacing: -0.8,
                              fontFamily: AccessibilityState.fontFamily),
        ...
        children: [
          if (today.isNotEmpty) ...[
            ...
            ...today.asMap().entries.map((e) => _NotifTile(
              ...
              onDismiss: () => NotificationState.instance.removeById(e.value.id),
            )),
          ],
    ...
    return AnimatedBuilder(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: navBg,
          border: Border(
              top: BorderSide(color: primary.withOpacity(0.06), width: 1)),
          boxShadow: [
            BoxShadow(
          ...
          return Column(mainAxisSize: MainAxisSize.min, children: [
            Transform.scale(
              ...
              child: Stack(clipBehavior: Clip.none, children: [
                ...
                if (showNotifBadge)
                  Positioned(
                      top: 2, right: 4, child: NavBadge(count: notifUnread)),
              ]),
            ),
            AnimatedContainer(
              duration: const Duration(milliseconds: 200),
...
class _NotifTileState extends State<_NotifTile>
  ...
  Widget build(BuildContext context) {
    ...
    final showConsentButtons = n.type == NotifType.chatConsent &&
        n.matchId != null &&
        !n.hasResponded;
                    ...
                    child: Row(
                        ...
                        children: [
                          ...
                          Padding(
                            ...
                            child: _NotifIcon(
                                type: n.type, accent: accent, isDark: isDark),
                          ),
...
class _ChatConsentButtons extends StatefulWidget {
  final String matchId;
  final Color accent;
  final VoidCallback onResponded;
  const _ChatConsentButtons(
      {required this.matchId, required this.accent, required this.onResponded});
  @override
  State<_ChatConsentButtons> createState() => _ChatConsentButtonsState();
}

class _ChatConsentButtonsState extends State<_ChatConsentButtons> {
  bool _loading = false;
  ...
  Future<void> _respond(bool accepted) async {
    ...
    try {
      ...
      final resp = await http.post(
        Uri.parse(
            'http://localhost:8080/api/matches/${widget.matchId}/chat-consent'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
      ...
      if (data['chatUnlocked'] == true) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('💬 Chat je otključan! Možete se dopisivati.'),
          backgroundColor: Color(0xFF700D25),
        ));
      } else if (!accepted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Odbili ste otvaranje razgovora.'),
        ));
      } else {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Pristali ste! Čekamo drugu osobu... ⏳'),
          backgroundColor: Color(0xFF700D25),
        ));
      }
      widget.onResponded();
      ...
      return const Center(
          child: SizedBox(
              ...
              child: CircularProgressIndicator(
                  strokeWidth: 2, color: Color(0xFF700D25))));
    }
    return Row(
      children: [
        Expanded(
...
class _NotifIcon extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final icon = switch (type) {
      ...
      NotifType.chatConsent => Icons.chat_bubble_outline_rounded,
      NotifType.chatUnlocked => Icons.lock_open_rounded,
      NotifType.chatDeclined => Icons.chat_bubble_outline_rounded,
