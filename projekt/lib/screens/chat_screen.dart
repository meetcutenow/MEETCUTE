...
class ChatState extends ChangeNotifier {
  ...
  Future<void> loadConversations() async {
    try {
      ...
      final backendConvs = await api.getConversations();

      conversations = backendConvs.map((c) {
        final existing = conversations.where((e) => e.id == c.id).firstOrNull;
...
class _ChatScreenState extends State<ChatScreen> with TickerProviderStateMixin {
  ...
  void _startConvRefresh() {
    _refreshTimer?.cancel();
    _refreshTimer = Timer.periodic(const Duration(seconds: 5), (_) {
      if (mounted) ChatState.instance.loadConversations();
    });
  }
          ...
          child: Container(
            ...
            child: Row(children: [
              Container(width: 52, height: 52,
                ...
                child: ClipOval(
                  child: convo.otherUserPhoto != null && convo.otherUserPhoto!.startsWith('http')
                      ? Image.network(convo.otherUserPhoto!, fit: BoxFit.cover,
...
class _ConvoState extends State<ChatConversationScreen> with TickerProviderStateMixin{
  ...
  Future<void> _loadMessages() async {
    try {
      ...
      final resp = await http.get(
        Uri.parse('http://localhost:8080/api/conversations/${widget.convo.id}/messages'),
      ...
      final messages = data.map((m) => ChatMessage(
        ...
        sentAt: m['sentAt'] != null ? DateTime.tryParse(m['sentAt']) : null,
      )).toList();

      final idx = ChatState.instance.conversations
    ...
    try {
      ...
      for (final m in newMessages) {
        ...
        _addMessage(ChatMessage(
          text: m['body'] as String?,
          isMe: false,
          sentAt: m['sentAt'] != null ? DateTime.tryParse(m['sentAt']) : DateTime.now(),
        ));
      }

      if (newMessages.any((m) => m['senderId'] != myId)) {
        ...
        child: AnimatedContainer(
          ...
          child: Row(children: [
            ...
            AnimatedContainer(
              ...
              child: widget.convo.otherUserPhoto != null
                  ? ClipOval(child: Image.network(
                  widget.convo.otherUserPhoto!,
                  fit: BoxFit.cover,
                  errorBuilder: (_, __, ___) => Icon(Icons.person_rounded, color: primary, size: 23)))
                  : Icon(Icons.person_rounded, color: primary, size: 23),
            ),
  ...
  Widget _avatar() {
    ...
    final primary = isDark ? kDarkPrimary : kLightPrimary;
    final accent  = isDark ? const Color(0xFF5A3A48) : kPrimaryLight;
    final photoUrl = widget.convo.otherUserPhoto;
    return Container(
      width: 30, height: 30,
