...
class _CompanySettingsScreenState extends State<CompanySettingsScreen>
      ...
      builder: (ctx) => Dialog(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(
                color: _primary.withOpacity(0.18), blurRadius: 30, offset: const Offset(0, 10))],
          ),
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Container(
              width: 60, height: 60,
              decoration: BoxDecoration(
                color: _primary.withOpacity(0.10), shape: BoxShape.circle,
            ...
            Row(children: [
              Expanded(child: GestureDetector(
                ...
                child: Container(
                  height: 46,
                  decoration: BoxDecoration(
                    color: _accent, borderRadius: BorderRadius.circular(23),
                    border: Border.all(color: _primary.withOpacity(0.25)),
                  ),
              ...
              Expanded(child: GestureDetector(
                ...
                child: Container(
                  height: 46,
                  decoration: BoxDecoration(
                    color: _primary, borderRadius: BorderRadius.circular(23),
                    boxShadow: [BoxShadow(
                        color: _primary.withOpacity(0.30), blurRadius: 12, offset: const Offset(0, 4))],
                  ),
  ...
  void _openEditProfile() => showDialog(
    ...
    builder: (_) => _ProfileEditDialog(
      primary: _primary, accent: _accent, card: _card, onPrimary: _onPrimary,
      onSaved: () => setState(() {}),
    ),
  );
          ...
          AnimatedContainer(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Postavke', style: TextStyle(
                    color: _primary, fontSize: 22, fontWeight: FontWeight.w900, letterSpacing: -0.5)),
                ...
                child: Row(children: [
                  Container(
                    ...
                    child: company.logoUrl != null && company.logoUrl!.isNotEmpty
                        ? ClipOval(child: Image.network(company.logoUrl!, fit: BoxFit.cover,
                        errorBuilder: (_, __, ___) =>
                            Icon(Icons.business_rounded, color: _primary, size: 28)))
                        : Icon(Icons.business_rounded, color: _primary, size: 28),
                  ),
              ...
              GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: [BoxShadow(
                        color: _primary.withOpacity(0.06), blurRadius: 12, offset: const Offset(0, 3))],
                  ),
    ...
    child: AnimatedContainer(
      ...
      child: Row(children: [
        AnimatedContainer(
          ...
          width: 44, height: 44,
          decoration: BoxDecoration(
              color: _accent, borderRadius: BorderRadius.circular(13),
              border: Border.all(color: _primary.withOpacity(0.15))),
          ...
          child: Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(subtitle, style: TextStyle(
                  color: _primary.withOpacity(0.50), fontSize: 12.5)),
            ])),
...
class _ProfileEditDialog extends StatefulWidget {
  ...
  const _ProfileEditDialog({
    ...
    required this.card, required this.onPrimary,
...
class _ProfileEditDialogState extends State<_ProfileEditDialog> {
  ...
  Future<void> _pickLogo() async {
    ...
    setState(() { _newLogoPath = xFile.path; _error = null; });
  }

  void _removeLogo() => setState(() { _newLogoPath = null; _logoUrl = null; });

  Future<void> _save() async {
    final orgName = _orgNameCtrl.text.trim();
    final email   = _emailCtrl.text.trim();

    if (orgName.length < 2) { setState(() => _error = 'Naziv mora imati najmanje 2 znaka.'); return; }
    if (!RegExp(r'^[\w\.\+\-]+@[\w\-]+\.[a-zA-Z]{2,}$').hasMatch(email)) {
      setState(() => _error = 'Unesite ispravnu email adresu.'); return;
    }
    ...
    try {
      ...
      if (_newLogoPath != null) {
        final request = http.MultipartRequest(
            'POST', Uri.parse('$_baseUrl/upload?folder=meetcute/logos'));
        request.headers['Authorization'] = 'Bearer $token';
        request.files.add(http.MultipartFile.fromBytes(
          'file', await File(_newLogoPath!).readAsBytes(),
          filename: _newLogoPath!.split('/').last,
          contentType: MediaType('image', 'jpeg'),
        ));
        final uploadResp = await http.Response.fromStream(
            await request.send().timeout(const Duration(seconds: 30)));

        if (uploadResp.statusCode == 200) {
          finalLogoUrl = (jsonDecode(utf8.decode(uploadResp.bodyBytes))['data']
          as Map<String, dynamic>)['url'] as String;
        } else {
          setState(() { _error = 'Upload loga nije uspio. Pokušaj ponovo.'; _saving = false; });
          return;
        }
      }

      final bodyMap = <String, String>{'orgName': orgName, 'email': email};
      if (finalLogoUrl != null && finalLogoUrl.isNotEmpty) bodyMap['logoUrl'] = finalLogoUrl;
      ...
      if (resp.statusCode == 200 && decoded['success'] == true) {
        final companyData = decoded['data'] as Map<String, dynamic>? ?? {};
        ...
        setState(() => _error = decoded['message'] as String? ?? 'Greška pri spremanju.');
      }
    } catch (_) {
  ...
  Widget build(BuildContext context) {
    ...
    final c  = widget.card;
    final hasLogo = _newLogoPath != null || (_logoUrl != null && _logoUrl!.isNotEmpty);

    Widget logoChild;
    if (_newLogoPath != null) {
      logoChild = Image.file(File(_newLogoPath!), fit: BoxFit.cover, width: 80, height: 80);
    } else if (_logoUrl != null && _logoUrl!.isNotEmpty) {
      logoChild = Image.network(_logoUrl!, fit: BoxFit.cover, width: 80, height: 80,
          errorBuilder: (_, __, ___) => Icon(Icons.business_rounded, color: p, size: 36));
    } else {
    ...
    return Dialog(
      ...
      child: TweenAnimationBuilder<double>(
        ...
        child: Container(
          decoration: BoxDecoration(
            color: c, borderRadius: BorderRadius.circular(26),
            border: Border.all(color: p.withOpacity(0.18)),
            boxShadow: [BoxShadow(
                color: p.withOpacity(0.22), blurRadius: 36, offset: const Offset(0, 14))],
          ),
          child: SingleChildScrollView(
            physics: const BouncingScrollPhysics(),
            padding: EdgeInsets.fromLTRB(22, 22, 22, mq.viewInsets.bottom + 22),
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              Row(children: [
                Container(
                  width: 40, height: 40,
                  decoration: BoxDecoration(
                      color: p.withOpacity(0.10), borderRadius: BorderRadius.circular(12)),
                  child: Icon(Icons.business_center_rounded, color: p, size: 20),
                ),
                const SizedBox(width: 12),
                Expanded(child: Text('Podaci organizacije', style: TextStyle(
                ...
                GestureDetector(
                  ...
                  child: Container(
                    width: 32, height: 32,
                    decoration: BoxDecoration(
                        color: p.withOpacity(0.08), borderRadius: BorderRadius.circular(10)),
              ...
              Row(children: [
                Stack(clipBehavior: Clip.none, children: [
                  GestureDetector(
                    ...
                    child: Container(
                      width: 80, height: 80,
                      decoration: BoxDecoration(
                        color: p.withOpacity(0.10), shape: BoxShape.circle,
                        border: Border.all(color: p.withOpacity(0.25), width: 2),
                      ),
                  ...
                  Positioned(bottom: 0, right: 0, child: GestureDetector(
                    ...
                    child: Container(
                      width: 28, height: 28,
                      decoration: BoxDecoration(
                          color: p, shape: BoxShape.circle,
                          border: Border.all(color: c, width: 2)),
                      child: const Icon(Icons.camera_alt_rounded, color: Colors.white, size: 14),
                    ),
                  )),
                  if (hasLogo) Positioned(top: 0, right: 0, child: GestureDetector(
                    onTap: _removeLogo,
                    child: Container(
                      width: 24, height: 24,
                      decoration: BoxDecoration(
                ...
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('Logo organizacije', style: TextStyle(
                      color: p, fontSize: 14, fontWeight: FontWeight.w700)),
                  const SizedBox(height: 4),
                  Text('Klikni na sliku za promjenu',
                      style: TextStyle(color: p.withOpacity(0.50), fontSize: 12)),
                ])),
              ]),
              const SizedBox(height: 20),
              ...
              const SizedBox(height: 8),
              _inputField(ctrl: _orgNameCtrl, hint: 'npr. EventCo d.o.o.',
                  icon: Icons.business_rounded, primary: p, card: c),
              const SizedBox(height: 14),
              _fieldLabel('Kontakt email *', p),
              const SizedBox(height: 8),
              _inputField(ctrl: _emailCtrl, hint: 'info@eventco.hr',
                  icon: Icons.email_outlined, primary: p, card: c,
                  keyboard: TextInputType.emailAddress),
              const SizedBox(height: 6),
              Row(children: [
                Icon(Icons.info_outline_rounded, color: p.withOpacity(0.40), size: 13),
                const SizedBox(width: 5),
                Expanded(child: Text(
                    'Ovaj email bit će prikazan korisnicima na tvojim događajima.',
                    style: TextStyle(color: p.withOpacity(0.45), fontSize: 11.5))),
              ]),
              if (_error != null) ...[
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: _saving ? [] : [BoxShadow(
                        color: p.withOpacity(0.35), blurRadius: 16, offset: const Offset(0, 6))],
                  ),
                  child: Center(child: _saving
                    ...
                    const SizedBox(width: 8),
                    Text('Spremi promjene', style: TextStyle(
  ...
  Widget _inputField({
    ...
    required IconData icon, required Color primary, required Color card,
    TextInputType? keyboard,
      ...
      Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [BoxShadow(
              color: primary.withOpacity(0.05), blurRadius: 10, offset: const Offset(0, 3))],
        ),
        child: Row(children: [
          const SizedBox(width: 14),
